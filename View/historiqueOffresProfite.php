...
class historiqueOffresProfite {
    public function entetePage() {
        ?>
        <head>
            <title>Mes Offres Profitées</title>
            ...
            <body class="to">
                ...
                <div class="content">
                    ...
                    <div class="subContent">
                        <?php $r->historySideBar("Mes offres profités"); ?>
                        <div class="users">
                            
                            <table id="all">
                                <tr class="head">
                                    <th onclick="sortTable('offre')">Offre</th>
                                    <th onclick="sortTable('offreT')">Type Offre</th>
                                    <th onclick="sortTable('nomP')">Partenaire</th>
                                    <th onclick="sortTable('catP')">Categorie</th>
                                    <th onclick="sortTable('villeP')">Ville</th>
                ...
                <script>
                    ...
                    function fetchOffresProfite() {

                        $.ajax({
                            url: 'index.php?router=OffresProfite',
                            type: 'GET',
                            dataType: 'json',
                            success: function(data) {
                                
                                $('#all tr:not(.head)').remove();
                                const offresTable = $('#all');
                                data.forEach(function(offre) {
                                    offresTable.append('<tr>' +
                                        '<td>' + offre.offre_nom + '</td>' +
                                        '<td>' + offre.offre_type + '</td>' +
                                        '<td>' + offre.partenaire_nom + '</td>' +
                                        '<td>' + offre.partenaire_categorie + '</td>' +
                                        '<td>' + offre.partenaire_ville + '</td>' +
                                        '<td>' + offre.dateProfit + '</td>' +
                                    '</tr>');
                                });
                            },
                    ...
                    function getColumnIndex(column) {
                        switch (column) {
                            case 'offre': return 0;
                            case 'offreT': return 1;
                            case 'nomP': return 2;
                            case 'catP': return 3;
                            case 'villeP': return 4;
                            case 'date': return 5;
                            default: return 0;
                        }
                    }
